...
public:
    ...
    bool SolveReverse(int row, int col);
    int solCount;

private:
    int puzzle[9][9];
    int puzzleReverse[9][9];
    bool compareAnswers();
    ...
    bool row_validR(int row) const;

    // Private member function that checks if the named column is valid
    bool col_validR(int col) const;

    // Private member function that checks if the named 3x3 block is valid
    bool block_validR(int row, int col) const;
};
